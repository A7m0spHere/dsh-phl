...
2026-09-12 alpha.5 发布轮：**`v0.1.0-alpha.5` 已发布，四类证据（哈希 / minisign / 更新清单 / 门禁）全部核验通过。** 真机缺陷（保留端口、cordis 数组、tooltip 裁切、固定端口自冲突）在本轮先修完合入，随后发布流水线首次独立跑通机器门禁——安装器冒烟门禁连拦三次，逐层暴露的都是**门禁自身**的缺陷：① 产物定位假定 `PHL.exe` 且不剥注册表引号（模板实际装 `dsh-phl.exe`，值带引号，`9667f6c`）；② 提权宿主下 WebView2 绝不开 DevTools 端口（普通权限 ~2 秒可用，管理员控制台与 CI runner 永不，`--no-sandbox` 无用）——改为 `Interactive+Limited` 计划任务 + `.bat` broker 启动应用并把 `PHL_ROOT` 带过任务边界（`7fe99ff`）；③ 托管 runner 即便应用与 5 个 WebView2 子进程都起来，端口仍 `bind=0`——故渲染三步（页目标 / IPC / console）以 `--no-gui` **显式 SKIP** 并注明移交真机车道，其余 8 步保持阻塞（`15b2eec`），真机车道同一脚本 11/11 通过并入库报告（`e3ebf41` + `docs/alpha5-installer-smoke-manual-2026-09-12.json`）。附带修掉一处用户可见缺陷：「版本」页「最新」徽章原先跟随 npm 的滞后 dist-tag（rc.2 已发布、徽章仍在 rc.1），现改为跟随目录最高版本，与列表排序、创建向导默认选中三者同源（`46a6fc1`，3 条单测）。链路：CI run `34677702589` 全绿 → tag `v0.1.0-alpha.5` = `c15fa82` → Release run `34678169041` 成功（22.4 分钟）→ 安装包 SHA-256 `B205ED07…4C16` 三方一致、minisign（keyid `aef74d979fa34850`，ED 预哈希）验签通过、`updates` 与在线端点均为 0.1.0-alpha.5。详见 [本轮发布验收](docs/alpha-release-acceptance-2026-09-12.md)。
